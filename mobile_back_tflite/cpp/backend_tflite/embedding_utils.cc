...
bool TsEmbeddingParser::parse(std::ifstream& fin, size_t file_size) {
    ...
    for (int32_t idx = 0; idx < steps; idx++) {
        ...
        if (!fin.read(reinterpret_cast<char*>(ts_embedding_seq_map_[steps][idx].data()),
...
std::vector<float> TsEmbeddingParser::get_timestamp_embedding(int32_t steps, 
...
bool EmbeddingManager::load_timestamp_embeddings(const std::string& filename) {
    ...
    if (!success) {
        std::cerr << "Failed to parse timestamp embeddings from " << filename << std::endl;
...
std::vector<float> EmbeddingManager::get_timestamp_embedding(int32_t timestep, 
                                                           int num_steps) const {
    if (!ts_parser_) {
