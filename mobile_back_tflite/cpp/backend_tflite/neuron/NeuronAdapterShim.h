...
inline void* loadAdapterLibrary(const char* name) {
  ...
  if (sHandle == nullptr) {
    ...
    auto getVerson = reinterpret_cast<decltype(&Neuron_getVersion)>(dlsym(sHandle, "Neuron_getVersion"));
    if (getVerson == nullptr || getVerson(&version) != NEURON_NO_ERROR) {
      LOG(ERROR) << "Loaded " << name << " with unknown version.";
    }else{
      LOG(INFO) << "Loaded " << name << " with version: "
                << (int)version.major << "."
                << (int)version.minor << "."
                << (int)version.patch;
    }
...
inline void* loadAdapterFunction(const char* name) {
  ...
  if (fn == nullptr) {
    LOG(ERROR) << "Unable to open function " << name;
  }
...
typedef int (*Neuron_getFeatureSupportedStatus_fn)(NeuronFeatureType type,
                                                   bool* supported);
...
typedef int (*NeuronModel_addOperationExtension_fn)(
    ...
    const NeuronDevice* device, uint32_t inputCount, const uint32_t* inputs,
...
typedef int (*NeuronModel_suppressInputConversion_fn)(NeuronModel* model,
                                                      bool suppress);

typedef int (*NeuronModel_suppressOutputConversion_fn)(NeuronModel* model,
                                                       bool suppress);

typedef int (*NeuronModel_restoreFromCompiledNetwork_fn)(
    NeuronModel** model, NeuronCompilation** compilation, const void* buffer,
    const size_t size);
...
typedef int (*NeuronCompilation_setTrimIOAlignment_fn)(
    NeuronCompilation* compilation, bool enable);
...
typedef int (*NeuronDebug_setReportPath_fn)(NeuronModel* model,
                                            const char* path);
...
typedef int (*NeuronDevice_getDescription_fn)(const NeuronDevice* device,
...
typedef int (*NeuronModel_restoreFromCompiledNetworkV2_fn)(
    NeuronModel** model, NeuronCompilation** compilation, const void* buffer,
    const size_t size, const CompilationType& type);

typedef int (*NeuronExecution_setRunnerPoolSize_fn)(NeuronExecution* execution,
                                                    uint8_t numRunners);

typedef int (*NeuronExecution_setBatchDone_fn)(NeuronExecution* execution);

typedef int (*NeuronExecution_submitInput_fn)(NeuronExecution* execution, uint32_t index, const void* buffer, size_t length);
typedef int (*NeuronExecution_setJobPoolSize_fn)(NeuronExecution* execution, uint32_t size);
typedef int (*NeuronExecution_queryOutput_fn)(NeuronExecution* execution, uint32_t index, void* buffer, size_t length);
typedef int (*NeuronExecution_queryOutputPointer_fn)(NeuronExecution* execution, uint32_t index, void**ptr);
typedef int (*NeuronExecution_flushExecution_fn)(NeuronExecution* execution);
typedef int (*NeuronExecution_resetParallelResources_fn)(NeuronExecution* execution);
...
inline int Neuron_getFeatureSupportedStatus(NeuronFeatureType type,
                                            bool* supported) {
...
inline int NeuronModel_addOperation(NeuronModel* model,
                                    NeuronOperationType type,
                                    uint32_t inputCount, const uint32_t* inputs,
                                    uint32_t outputCount,
                                    const uint32_t* outputs) {
  LOAD_ADAPTER_FUNCTION(NeuronModel_addOperation);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(model, type, inputCount, inputs,
                                      outputCount, outputs);
}
...
inline int NeuronModel_addOperationExtension(
  ...
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(model, name, vendor, device, inputCount,
                                      inputs, outputCount, outputs);
}

inline int NeuronModel_identifyInputsAndOutputs(NeuronModel* model,
                                                uint32_t inputCount,
                                                const uint32_t* inputs,
                                                uint32_t outputCount,
                                                const uint32_t* outputs) {
  LOAD_ADAPTER_FUNCTION(NeuronModel_identifyInputsAndOutputs);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(model, inputCount, inputs, outputCount,
                                      outputs);
}

inline int NeuronModel_getSupportedOperations(NeuronModel* model,
                                              bool* supported,
                                              uint32_t operationCount) {
...
inline int NeuronCompilation_create(NeuronModel* model,
                                    NeuronCompilation** compilation) {
  LOAD_ADAPTER_FUNCTION(NeuronCompilation_create);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(model, compilation);
}

inline int NeuronCompilation_createWithOptions(NeuronModel* model,
                                               NeuronCompilation** compilation,
                                               const char* options) {
...
inline int NeuronExecution_setCacheFlushHint(NeuronExecution* execution,
                                             uint8_t flushHint) {
  LOAD_ADAPTER_FUNCTION(NeuronExecution_setCacheFlushHint);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(execution, flushHint);
}
...
inline int NeuronCompilation_setCaching(NeuronCompilation* compilation,
                                        const char* cacheDir,
                                        const uint8_t* token) {
...
inline int NeuronCompilation_setPreference(NeuronCompilation* compilation,
                                           int32_t preference) {
  LOAD_ADAPTER_FUNCTION(NeuronCompilation_setPreference);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(compilation, preference);
}

inline int NeuronCompilation_setPriority(NeuronCompilation* compilation,
                                         int32_t priority) {
...
inline int NeuronCompilation_getInputPaddedSize(NeuronCompilation* compilation,
...
inline int NeuronCompilation_getOutputPaddedSize(NeuronCompilation* compilation,
                                                 int32_t index, size_t* size) {
...
inline int NeuronCompilation_setTrimIOAlignment(NeuronCompilation* compilation,
                                                bool enable) {
  LOAD_ADAPTER_FUNCTION(NeuronCompilation_setTrimIOAlignment);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(compilation, enable);
}

inline int NeuronCompilation_setSWDilatedConv(NeuronCompilation* compilation,
                                              bool enable) {
  LOAD_ADAPTER_FUNCTION(NeuronCompilation_setSWDilatedConv);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(compilation, enable);
}

inline int NeuronExecution_create(NeuronCompilation* compilation,
                                  NeuronExecution** execution) {
...
inline int NeuronExecution_setInputFromMemory(NeuronExecution* execution,
                                              uint32_t index,
                                              const NeuronOperandType* type,
                                              const NeuronMemory* memory,
                                              size_t offset, size_t length) {
  LOAD_ADAPTER_FUNCTION(NeuronExecution_setInputFromMemory);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(execution, index, type, memory, offset,
                                      length);
}
...
inline int NeuronExecution_setOutputFromMemory(NeuronExecution* execution,
                                               uint32_t index,
                                               const NeuronOperandType* type,
                                               const NeuronMemory* memory,
                                               size_t offset, size_t length) {
...
inline int NeuronMemory_createFromFd(size_t size, int protect, int fd,
...
inline int NeuronExecution_startComputeWithDependencies(
  ...
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(execution, dependencies, num_dependencies,
                                      duration, event);
}

inline int NeuronEvent_getSyncFenceFd(const NeuronEvent* event,
                                      int* syncFenceFd) {
...
inline int NeuronExecution_setLoopTimeout(NeuronExecution* execution,
                                          uint64_t duration) {
  LOAD_ADAPTER_FUNCTION(NeuronExecution_setLoopTimeout);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(execution, duration);
}

inline int NeuronExecution_setBoostHint(NeuronExecution* execution,
                                        uint8_t boostValue) {
...
inline int NeuronDevice_getDescription(const NeuronDevice* device,
                                       const char** description) {
  LOAD_ADAPTER_FUNCTION(NeuronDevice_getDescription);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(device, description);
}

inline int NeuronDevice_getExtensionSupport(const char* extensionName,
                                            bool* isExtensionSupported) {
...
inline int NeuronModel_getExtensionOperandType(
  ...
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(model, extensionName,
                                      operandCodeWithinExtension, type);
}
...
inline int NeuronModel_getExtensionOperationType(
  ...
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(model, extensionName,
                                      operationCodeWithinExtension, type);
}

inline int NeuronModel_setOperandExtensionData(NeuronModel* model,
...
inline int NeuronCompilation_createForBatch(NeuronModel* model,
                                            NeuronCompilation** compilation) {
...
inline int NeuronExecution_setRunnerPoolSize(NeuronExecution* execution,
                                             uint8_t numRunners) {
...
inline int NeuronExecution_submitInput(NeuronExecution* execution, uint32_t index, const void* buffer, size_t length){
  LOAD_ADAPTER_FUNCTION(NeuronExecution_submitInput);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(execution, index, buffer, length);
}
inline int NeuronExecution_queryOutput(NeuronExecution* execution, uint32_t index, void* buffer, size_t length){
  LOAD_ADAPTER_FUNCTION(NeuronExecution_queryOutput);
  EXECUTE_ADAPTER_FUNCTION_RETURN_INT(execution, index, buffer, length);
}
inline int NeuronExecution_queryOutputPointer(NeuronExecution* execution, uint32_t index, void**ptr){
...
inline int NeuronExecution_setJobPoolSize(NeuronExecution* execution, uint32_t size) {
