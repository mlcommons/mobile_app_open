...
extern "C" struct dart_ffi_run_benchmark_out* dart_ffi_run_benchmark(
  ...
  // TODO (anhappdev): Refactor the C++ code so that ComputeLatency() only report
  // duration in ms and not QPS (but after moving `android/cpp` out of `android` folder)
  // See https://github.com/mlcommons/mobile_app_open/issues/237
