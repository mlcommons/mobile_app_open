...
EnnModelId mdl_id[MAX_INSTANCE] = {
    INVALID_MDL,
};
...
bool sbe2200::impl_closeModel() {
  ...
  for (int i = 0; i < MAX_INSTANCE; i++) {
    if (mdl_id[i] == INVALID_MDL) continue;
...
void sbe2200::impl_set_buffer(int mdl_idx, void *buf, enn_buf_dir_e dir,
                              int idx) {
...
EnnBufferPtr sbe2200::impl_get_memobj(void *p) { return mapped_mem[p]; }
...
void sbe2200::impl_config_batch() {
  ...
  for (int mdl_idx = 0; mdl_idx < MAX_INSTANCE && mdl_idx < m_batch_size;
    ...
    task_thread_executor[mdl_idx] =
        std::thread(&sbe2200::impl_inference_thread, this, mdl_idx);
  }
...
void sbe2200::set_inbuf(void *p, int batch_idx, int idx) {
  if (m_batch_size == 1) {
    ...
    task_pool.push(
        {p, (char *)m_outbuf[0] + batch_idx * mdl_container->m_outbuf_size});
  }
}
...
void sbe2200::impl_parse_ext_attribute(
