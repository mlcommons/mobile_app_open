...
void notify_f(addr_t *addr, addr_t value) { *addr = value; }
...
void sbe1200::impl_inference_thread(int mdl_idx) {
  ...
  while (true) {
    ...
    inferece_start_cond[mdl_idx].wait(
        lock, [this] { return (!task_pool.empty() || force_thread_done); });
    MLOGD("thread unlock with mdl[%d]", mdl_idx);
    if (force_thread_done) return;

    std::pair<void *, void *> buf;
    while (task_deque(mdl_idx, buf)) {
      MLOGD("mdl_idx[%d], m_mdl_inbuf[%x], m_mdl_outbuf[%x]", mdl_idx,
            m_mdl_inbuf[mdl_idx], m_mdl_outbuf[mdl_idx]);
      MLOGD("m_mdl_inbuf[%d]->addr[%x], buf[%x]", mdl_idx,
            m_mdl_inbuf[mdl_idx]->addr, buf.first);
      ...
      if (ret != RET_OK) {
        ...
        if (callbacks[mdl_idx].waitFor(&callbacks[mdl_idx].requestId,
                                       requestId[mdl_idx],
                                       EDEN_NN_TIMEOUT) < 0) {
...
bool sbe1200::impl_closeModel() {
  int ret = RET_OK;
  for (int mdl_idx = 0; mdl_idx < MAX_INSTANCE && mdl_idx < m_batch_size;
       mdl_idx++) {
...
bool sbe1200::open_model(const char *model_path, const char *accelerator) {
  ...
  options.modelPreference = {{pref_hw, BOOST_MODE, {false, false}},
                             EDEN_NN_API};
  ...
  for (int mdl_idx = 0; mdl_idx < MAX_INSTANCE && mdl_idx < m_batch_size;
    ...
    ret = OpenEdenModelFromMemory(
        MODEL_TYPE_IN_MEMORY_TFLITE, reinterpret_cast<int8_t *>(m_mdl_buf),
        m_mdl_buf_len, false, &mdl_id[mdl_idx], options);
    if (ret != RET_OK) {
...
bool sbe1200::set_model_buf() {
  ...
  for (int32_t mdl_idx = 0; mdl_idx < MAX_INSTANCE && mdl_idx < m_batch_size;
       mdl_idx++) {
    for (int idx = 0; idx < mdl_container->m_in_cnt; idx++) {
      ret = AllocateInputBuffers(
          mdl_id[mdl_idx],
          &m_mdl_inbuf[mdl_container->m_in_cnt * mdl_idx + idx], &buf_cnt);
      if (ret != RET_OK) {
        MLOGE("fail to alloc inbuf mdl_idx[%d], ret = %d", 0, ret);
        return false;
      }
      MLOGD("m_mdl_inbuf[idx]->addr[%x], size[%d]",
            m_mdl_inbuf[mdl_container->m_in_cnt * mdl_idx + idx]->addr,
            m_mdl_inbuf[mdl_container->m_in_cnt * mdl_idx + idx]->size);
    }
    for (int idx = 0; idx < mdl_container->m_out_cnt; idx++) {
      ret = AllocateOutputBuffers(
          mdl_id[mdl_idx],
          &m_mdl_outbuf[mdl_container->m_out_cnt * mdl_idx + idx], &buf_cnt);
      if (ret != RET_OK) {
        MLOGE("fail to alloc outbuf mdl_idx[%d], ret = %d", 0, ret);
        return false;
      }
      MLOGD("m_mdl_outbuf[idx]->addr[%x], size[%d]",
            m_mdl_outbuf[mdl_container->m_out_cnt * mdl_idx + idx]->addr,
            m_mdl_outbuf[mdl_container->m_out_cnt * mdl_idx + idx]->size);
    }
  }

  MLOGD("m_mdl_inbuf.size[%d], m_mdl_outbuf.size[%d]", m_mdl_inbuf.size(),
        m_mdl_outbuf.size());
  return true;
}
...
bool sbe1200::inference() {
  ...
  if (m_batch_size == 1) {
    ...
    if (callbacks[0].waitFor(&callbacks[0].requestId, requestId[0],
                             EDEN_NN_TIMEOUT) < 0) {
...
void sbe1200::impl_config_batch() {
  ...
  for (int mdl_idx = 0; mdl_idx < MAX_INSTANCE && mdl_idx < m_batch_size;
       mdl_idx++) {
    task_thread_executor[mdl_idx] =
        std::thread(&sbe1200::impl_inference_thread, this, mdl_idx);
  }
...
void sbe1200::set_inbuf(void *p, int batch_idx, int idx) {
  if (m_batch_size == 1) {
    ...
    task_pool.push(
        {p, (char *)(m_batch_buf) + batch_idx * mdl_container->m_outbuf_size});
  }
}
...
void sbe1200::impl_parse_ext_attribute(
...
void sbe1200::impl_parse_mdl_attribute() {
  ...
  mdl_attr.m_outbuf_size =
      out_w * out_h * out_c * out_n * mdl_attr.get_byte(mdl_attr.m_outbuf_type);
}
...
bool backend_create(const char *model_path,
                    mlperf_backend_configuration_t *configs,
...
mlperf_status_t backend_get_output(uint32_t batch_idx, int32_t idx,
                                   void **data) {
...
void backend_delete() { sbe_obj.clear(); }

void *backend_get_buffer(size_t size) { return sbe_obj.allocate_buf(size); }

void backend_release_buffer(void *p) { sbe_obj.release_buf(p); }
