...
enum ACCER_IDX { NPU = 0, GPU, DSP, NPUDSP, MAX_ACCER_IDX };
...
typedef struct _config_info {
  ...
  int get_size() { return n * h * w * c; }
} config_info;

#define UPDATE_CONFIG(a, b) a = b ? b : 1;

#define UPDATE_MDL(a, b) a = b ? b : a;
...
class model_container {
 ...
 public:
  ...
  virtual void init() = 0;
  virtual void *get_ofm(std::vector<void *> &, uint32_t, int32_t) {
    return nullptr;
  }
  virtual void *get_ofm_eden(void *, void *, uint32_t, int32_t) {
    return nullptr;
  }
...
class classification : public model_container {
 public:
  ...
  void *get_ofm(std::vector<void *> &ofm_buf, uint32_t batch_idx,
                int32_t idx) override {
    return ofm_buf[0];
  }

 public:
  classification(config_info config)
      : model_container(config, 1, 1, IMAGE_CLASSIFICATION) {}
};
...
class classification_offline : public model_container {
 ...
 public:
  classification_offline(config_info config)
      : model_container(config, 1, 1, IMAGE_CLASSIFICATION) {}
};
...
class segmentation : public model_container {
 ...
 public:
  segmentation(config_info config)
      : model_container(config, 1, 1, IMAGE_SEGMENTATION) {}
};
class super_resolution : public model_container {
 public:
  void init() override {
    ...
    for (int i = 0; i < m_out_cnt; i++)
      set_output_data(
          {m_outbuf_type,
           (int64_t)(m_outbuf_size /
                     mdl_config->get_byte(mdl_config->m_outbuf_type))});
  }
  void *get_ofm(std::vector<void *> &ofm_buf, uint32_t batch_idx,
                int32_t idx) override {
    return ofm_buf[0];
 ...
 public:
  super_resolution(config_info config)
      : model_container(config, 1, 1, SUPER_RESOLUTION) {}
};
class detection : public model_container {
 ...
 public:
  detection(config_info config)
      : model_container(config, 1, 1, OBJECT_DETECTION),
        det_block_cnt(10),
        det_block_size(7) {}
  ~detection() {
    det_lbl_boxes.clear();
...
model_container *get_mdl_container() {
  ...
  for (auto e : configuration) {
    if (mdl_config->m_inbuf_size ==
        e.first.get_size() * mdl_config->get_byte(mdl_config->m_inbuf_type)) {
      auto config = e.first;
      switch (e.second) {
...
class classification : public model_container {
 public:
  ...
  void *get_ofm_eden(void *ofm_buf, void *batch_buf, uint32_t batch_idx,
                     int32_t idx) override {
    return ofm_buf;
  }
...
class detection : public model_container {
 ...
 public:
  detection(config_info config)
      : model_container(config, 1, 4, OBJECT_DETECTION),
        det_block_cnt(11),
        det_block_size(7) {}
  ~detection() {
    det_lbl_boxes.clear();
