...
namespace sbe
{
    ...
    static bool retrieve_model_surfix(const char *model, int core_id)
    {
        ...
        if (core_id == CORE_2200)
        {
            if (strstr((char *)model, "B") ||
                strstr((char *)model, "B/DS"))
                return true;
            else if (strstr((char *)model, "UNIVERSAL") ||
                     strstr((char *)model, "universal"))
                return true;
            else if (strstr((char *)model, "ERD") ||
                     strstr((char *)model, "erd"))
                return true;
        }
        else
        {
    ...
    static int get_core_id_from_model(const char *model)
    {
        ...
        if (strstr((char *)model, "ERD8825") ||
            ...
            core_id = CORE_2100;
                 ...
                 strstr((char *)model, "S908"))
            core_id = CORE_2200;
        else if (strstr((char *)model, "ERD9935") ||
                 strstr((char *)model, "S5E9935") ||
                 strstr((char *)model, "S919O"))
            core_id = CORE_2300;
        else
            return CORE_INVALID;
    ...
    int core_ctrl::support_sbe(const char *manufacturer, const char *model)
    {
        ...
        MLOGV("sbe version [%s]", version.c_str());
    ...
    const char *core_ctrl::get_benchmark_config(int core_id)
    {
        ...
        ;
        if (core_id == CORE_1200)
        {
            settings = sbe1200_config.c_str();
        }
        else if (core_id == CORE_2100)
        {
            settings = sbe2100_config.c_str();
        }
        else if (core_id == CORE_2200)
        {
            settings = sbe2200_config.c_str();
        }
        else if (core_id == CORE_2300)
        {
            settings = sbe2300_config.c_str();
        }
        return settings;
    }
