...
class sbe_core_holder {
	public:		
		...
		bool load_core_library(const char* lib_path)
		{
			char *error = nullptr;
			/* load sbe core */
			sbe::core_id = core_ctrl::get_core_id();
			MLOGD("acquired core id[%d]", sbe::core_id);

			if(core_id == CORE_INVALID) {
					MLOGE("fail to get sbe core libarary. core_id[%d]", sbe::core_id);
					return false;
			}

			std::string core_lib_path = std::string(lib_path) + "/" + sbe_core_libs[sbe::core_id];
